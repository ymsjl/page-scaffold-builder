...
export default function EntityTypeDesignerPanel({ open, onClose }: EntityTypeDesignerPanelProps) {
  const [editingId, setEditingId] = useState<string | null>(null);
  const [fieldEditableKeys, setFieldEditableKeys] = useState<React.Key[]>([]);

  const editingEntityType = useBuilderStore.use.editingEntityType();
  ...
  const handleSaveEntity = useCallback(() => {
    ...
    form.resetFields();
    onClose();
  }, [editingId, isTableEditing, onClose, form]);
        ...
        <Space direction="vertical" size="middle">
          ...
          <Space direction="vertical" size="small" style={{ width: '100%' }}>
            <EditableProTable<SchemaField>
              ...
              onChange={(nextFields: SchemaField[] | undefined) => {
