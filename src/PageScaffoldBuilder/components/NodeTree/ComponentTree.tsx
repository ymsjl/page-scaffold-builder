...
import { NormalizedComponentNode } from '../../types';
import TreeNodeItem from './TreeNodeItem';
import { useBuilderStore } from '../../store/useBuilderStore';
import { getAvailableComponents } from '../../componentMetas';
...
const ComponentTree: React.FC = () => {
  ...
  const selectNode = useBuilderStore(s => s.componentTree.selectNode);
  ...
  const treeNodes = useMemo<TreeDataNode[]>(() => {
    const buildNode = (node: NormalizedComponentNode): TreeDataNode => {
      return {
        key: node.id,
        title: <TreeNodeItem node={node} level={0} availableComponents={availableComponents} />,
        children: node.childrenIds?.map(childId => buildNode(nodesById[childId])) || [],
      };
    };
    return rootIds.map(id => buildNode(nodesById[id]));
  }, [nodesById, rootIds, availableComponents]);
