...
import { getComponentMeta } from '../componentMetas';
...
const ComponentPreview: React.FC<ComponentPreviewProps> = () => {
  ...
  const renderComponent = () => {
    if (typeof Component === 'string') {
      ...
      return React.createElement(htmlElement, { ...props, key: node.id }, children);
    }
    ...
    <div style={containerStyle}>
      <div style={previewContainerStyle}>
        <div style={headerStyle}>
          ...
          <span style={{ marginLeft: '8px', color: '#999' }}>({meta.name})</span>
        </div>
        {renderComponent()}
      </div>
    </div>
