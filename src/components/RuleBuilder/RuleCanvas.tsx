...
const RuleItem: React.FC<RuleItemProps> = memo(function RuleItem({
  ...
  const style = useMemo<React.CSSProperties>(
    () => ({ background: node.enabled ? undefined : "#fafafa", border: isSelected ? "1px solid #1677ff" : undefined }),
  ...
  const lengthOp = useMemo(() => getLengthDefaultOperator(node.params), [node.params]);
  const rangeOp = useMemo(() => getRangeDefaultOperator(node.params), [node.params]);
  const dateOp = useMemo(() => getDateDefaultOperator(node.type, node.params), [node.type, node.params]);
  ...
  return (
    <Card
      ...
      extra={
        <Space wrap>
          <button aria-label="delete" onClick={handleDelete} style={{ background: "none", border: "none", cursor: "pointer" }}>
            <DeleteOutlined />
          </button>
        </Space>
      }
              ...
              <Row gutter={8} wrap align="middle">
                <Col span={6}>
                  <Select
                    ...
                    onChange={(op: any) => {
                      if (op === "eq") {
                        const len = node.params?.len ?? node.params?.min ?? undefined;
                        updateParams({ operator: "eq", len, min: undefined, max: undefined });
                      } else if (op === "between") {
                        const min = node.params?.min ?? node.params?.len ?? undefined;
                        const max = node.params?.max ?? undefined;
                        updateParams({ operator: "between", min, max, len: undefined });
                      } else if (op === "gte") {
                        const min = node.params?.min ?? node.params?.len ?? undefined;
                        updateParams({ operator: "gte", min, max: undefined, len: undefined });
                      } else if (op === "lte") {
                        const max = node.params?.max ?? node.params?.len ?? undefined;
                        updateParams({ operator: "lte", max, min: undefined, len: undefined });
                      }
                    }}
                    options={[
                ...
                <Col span={18}>
                  {((node.params?.operator ?? lengthOp) === "eq") ? (
                    <InputNumber style={{ width: "100%", marginBottom: 6 }} placeholder="长度" value={node.params?.len} onChange={(val) => updateParams({ len: val ?? undefined })} />
                  ) : (
                    <Row gutter={8} wrap align="middle">
                      <Col span={12}>
                        <InputNumber style={{ width: "100%", marginBottom: 6 }} placeholder="最小长度" value={node.params?.min} onChange={(val) => updateParams({ min: val ?? undefined })} />
                      </Col>
                      <Col span={12}>
                        <InputNumber style={{ width: "100%", marginBottom: 6 }} placeholder="最大长度" value={node.params?.max} onChange={(val) => updateParams({ max: val ?? undefined })} />
                      </Col>
                    </Row>
                  )}
                </Col>
              ...
              <Row gutter={8} wrap align="middle">
                <Col span={6}>
                  <Select
                    ...
                    onChange={(op: any) => {
                      if (op === "eq") {
                        const v = node.params?.min ?? node.params?.max ?? undefined;
                ...
                <Col span={18}>
                  {((node.params?.operator ?? rangeOp) === "eq") ? (
                    <InputNumber style={{ width: "100%", marginBottom: 6 }} placeholder="值" value={node.params?.min ?? node.params?.max} onChange={(val) => updateParams({ min: val ?? undefined, max: val ?? undefined })} />
                  ) : ((node.params?.operator ?? rangeOp) === "between") ? (
                    <Row gutter={8} wrap align="middle">
                      <Col span={12}>
                        <InputNumber style={{ width: "100%", marginBottom: 6 }} placeholder="最小值" value={node.params?.min} onChange={(val) => updateParams({ min: val ?? undefined })} />
                      </Col>
                      <Col span={12}>
                        <InputNumber style={{ width: "100%", marginBottom: 6 }} placeholder="最大值" value={node.params?.max} onChange={(val) => updateParams({ max: val ?? undefined })} />
                      </Col>
                    </Row>
          ...
          {node.type === "pattern" ? (
            <>
              <Row gutter={8}>
                ...
                <Col span={18}>
                  <Input placeholder="自定义正则表达式" value={node.params?.pattern || ""} onChange={(e) => updateParams({ pattern: e.target.value })} disabled={patternSelectedKey !== "custom"} style={{ marginBottom: 6 }} />
                </Col>
              </Row>
            </>
          ) : null}

          {node.type === "dateMin" || node.type === "dateMax" || node.type === "dateRange" ? (
            <>
              <Row gutter={8} wrap align="middle">
                <Col span={6}>
                  <Select
                    ...
                    onChange={(op: any) => {
                      if (op === "eq") {
                        const v = node.params?.minDate ?? node.params?.maxDate ?? undefined;
                        updateParams({ operator: "eq", minDate: v, maxDate: v });
                      } else if (op === "between") {
                        ...
                        const minDate = node.params?.minDate ?? undefined;
                        updateParams({ operator: "gte", minDate, maxDate: undefined });
                      } else if (op === "lte") {
                        const maxDate = node.params?.maxDate ?? undefined;
                        updateParams({ operator: "lte", maxDate, minDate: undefined });
                      }
                    }}
                ...
                <Col span={18}>
                  {((node.params?.operator ?? dateOp) === "eq") ? (
                    <DatePicker style={{ width: "100%", marginBottom: 6 }} value={node.params?.minDate ? dayjs(node.params.minDate) : undefined} onChange={(date) => updateParams({ minDate: date ? date.format("YYYY-MM-DD") : undefined, maxDate: date ? date.format("YYYY-MM-DD") : undefined })} />
                  ) : ((node.params?.operator ?? dateOp) === "between") ? (
                    <Row gutter={8} wrap align="middle">
                      <Col span={12}>
                        <DatePicker style={{ width: "100%", marginBottom: 6 }} placeholder="开始日期" value={node.params?.minDate ? dayjs(node.params.minDate) : undefined} onChange={(date) => updateParams({ minDate: date ? date.format("YYYY-MM-DD") : undefined })} />
                      </Col>
                      <Col span={12}>
                        <DatePicker style={{ width: "100%", marginBottom: 6 }} placeholder="结束日期" value={node.params?.maxDate ? dayjs(node.params.maxDate) : undefined} onChange={(date) => updateParams({ maxDate: date ? date.format("YYYY-MM-DD") : undefined })} />
                      </Col>
                    </Row>
...
export default memo(function RuleCanvas() {
  ...
  const updateNode = useCallback((n: RuleNode) => dispatch(ruleBuilderActions.updateNode(n)), [dispatch]);
  const deleteNode = useCallback((id: string) => dispatch(ruleBuilderActions.deleteNode(id)), [dispatch]);
      ...
      <div style={{ borderRadius: 8, padding: 4 }}>
        <Space direction="vertical" style={{ width: "100%" }} size={8}>
          {nodes.length === 0 ? (
            ...
            nodes.map((node) => (
              <RuleItem key={node.id} node={node} isSelected={node.id === selectedId} onDelete={deleteNode} onChange={updateNode} />
            ))
          )}
        </Space>
      </div>
