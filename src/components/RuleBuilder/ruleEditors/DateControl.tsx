...
export default function DateControl({ value, onChange, placeholder }: { value?: any; onChange?: (v: any) => void; placeholder?: string }) {
  const mode = React.useMemo(() => {
    if (typeof value === 'string' || dayjs.isDayjs(value)) {
      return 'absolute';
      ...
      <Col span={8}>
        <Select
          ...
          onChange={(val) => {
            ...
            onChange(val === 'absolute' ? "" : { type: 'relative', preset: 'today' })
          }}
      ...
      <Col span={16}>
        {m === 'absolute' ? (
          <DatePicker
            ...
            value={typeof value === 'string' && value ? dayjs(value) : (dayjs.isDayjs(value) ? value : undefined)}
            onChange={(date) => onChange && onChange(date ? date.format('YYYY-MM-DD') : undefined)}
          />
        ) : (
          <Row gutter={8} align="middle">
            <Col span={12}>
              <Select
                ...
                onChange={(preset) => onChange && onChange({ type: 'relative', preset })}
            ...
            <Col span={12}>
              <InputNumber
                ...
                placeholder="偏移 (天)"
                value={value?.offset}
                onChange={(v) => onChange && onChange({ type: 'relative', preset: value?.preset ?? 'today', offset: v ?? undefined })}
              />
            </Col>
          </Row>
        )}
