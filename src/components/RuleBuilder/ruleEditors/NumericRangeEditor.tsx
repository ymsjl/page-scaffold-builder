...
const NumericRangeEditor: React.FC<Props> = React.memo(({ params, updateParams }) => {
  const rangeOp = React.useMemo(() => getRangeDefaultOperator(params), [params]);
  const operator = params?.operator ?? rangeOp;

  const handleOperatorChange = React.useCallback(
    (op: string) => updateParams(computeOperatorParams(op, params, "min", "max")),
    [updateParams, params]
  );

  const inputRender = useCallback(({ key, placeholder, value, onChange }: {
    ...
    const fallbackOnChange = (key ? (val: number | undefined) => updateParams({ [key]: val ?? undefined }) : undefined);
    return (
      <InputNumber
        style={inputStyle}
  ...
  const rangeInputElem = React.useMemo(() => {
    switch (operator) {
      case "eq":
        ...
        const onChange = (val: number | undefined) => updateParams({ min: val ?? undefined, max: val ?? undefined });
