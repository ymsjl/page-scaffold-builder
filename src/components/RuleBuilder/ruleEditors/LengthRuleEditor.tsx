...
const LengthRuleEditor: React.FC<Props> = React.memo(({ params, updateParams }) => {
  ...
  const handleOperatorChange = useCallback(
    (op: string) => {
      if (op === "eq") {
        ...
        updateParams({ len: nextLen, min: undefined, max: undefined, operator: op });
      } else {
  ...
  const inputRender = useCallback(
    ({ key, placeholder, value }: { key?: string; placeholder: string; value?: number }) => {
      const onChange = (val: number | undefined) => updateParams({ ...params, [key as string]: val });
      return (
        <InputNumber
          style={inputStyle}
          value={value}
  ...
  const lengthInputElem = React.useMemo(() => {
    switch (operator) {
      ...
      case "gte":
        return inputRender({ key: "min", placeholder: "最小长度", value: params?.min });
      case "lte":
        return inputRender({ key: "max", placeholder: "最大长度", value: params?.max });
      case "between":
      default:
        const paramMetas = [
  ...
  }, [operator, params?.min, params?.max, params?.len, updateParams, inputRender]);
