...
describe('ruleMapping operator inference', () => {
  ...
  it('range (date) validator validates single date against min/max (absolute)', async () => {
    const nodes: any = [
      { id: 'd2', type: 'range', enabled: true, params: { valueType: 'date', minDate: '2021-05-01', maxDate: '2021-05-10' }, message: 'single date range' },
    ];
    const rules = nodesToRules(nodes) as any[];
    expect(rules.length).toBe(1);
    const validator = rules[0].validator as Function;
  ...
  it('range (date) validator validates single date against relative min (today)', async () => {
    const nodes: any = [
      { id: 'd3', type: 'range', enabled: true, params: { valueType: 'date', operator: 'gte', minDate: { type: 'relative', preset: 'today' } }, message: 'relative min' },
    ];
    const rules = nodesToRules(nodes) as any[];
    const validator = rules[0].validator as Function;
    const today = dayjs().format('YYYY-MM-DD');
    await expect(validator({}, today)).resolves.toBeUndefined();
    await expect(validator({}, dayjs().add(-1, 'day').format('YYYY-MM-DD'))).rejects.toBeDefined();
  });
});
