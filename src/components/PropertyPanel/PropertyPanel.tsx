...
const PropertyPanel: React.FC = () => {
  ...
  const propAttrs = useMemo(() => {
    ...
    return Object.values(getComponentPrototype(selectedComponentType)?.propsTypes ?? {}).map(item => ({
      ...item,
      ...(item.name === 'entityTypeId'
        ? { options: entityTypes.map(et => ({ name: et.name, value: et.id, label: et.name })) }
        : {}),
    }));
  ...
  const renderFormItem = (propAttr: PropAttribute) => {
    ...
    if (propAttr.name === 'columns') {
      return (
        <ProForm.Item key={propAttr.name} label={propAttr.label} tooltip={propAttr.description}>
    ...
    switch (propAttr.type) {
      case 'select':
        return (<ProFormSelect key={propAttr.name} {...formItemConfig} options={propAttr?.options || []} />);
      case 'boolean':
