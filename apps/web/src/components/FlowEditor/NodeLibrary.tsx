...
import * as styles from './NodeLibrary.css';
...
export const NodeLibrary: React.FC<NodeLibraryProps> = ({ onNodeSelect }) => {
  ...
  // 处理拖拽开始
  const handleDragStart = (e: React.DragEvent, nodeType: ActionNodeType) => {
  ...
  return (
    <div className={styles.nodeLibrary}>
      ...
      <div className={styles.nodeLibraryContent}>
        <Collapse defaultActiveKey={Object.keys(nodesByCategory)} ghost expandIconPosition="end">
                ...
                {nodes.map((node) => (
                  ...
                  >
                    <div className={styles.nodeCardContent}>
                      <span className={styles.nodeCardIcon}>{node.icon}</span>
                      <div className={styles.nodeCardInfo}>
                        <div className={styles.nodeCardName}>{node.name}</div>
        ...
        {filteredNodes.length === 0 && (
          <div className={styles.nodeLibraryEmpty}>
            <p>未找到匹配的节点</p>
          </div>
        )}
      </div>
