...
const DateRangeSpanEditor: React.FC<Props> = React.memo(
  ({ params, updateParams }) => {
    ...
    const handleOperatorChange = React.useCallback(
      (op: string) =>
        updateParams(computeOperatorParams(op, params, "minSpan", "maxSpan")),
      [updateParams, params],
    );
      ...
      switch (operator) {
        case "eq": {
          ...
          const onChange = (val: number | undefined) =>
            updateParams({
              minSpan: val ?? undefined,
              maxSpan: val ?? undefined,
            });
