...
const DateRangeEditor: React.FC<Props> = React.memo(
  ({ params, updateParams }) => {
    const rangeOp = React.useMemo(
      () => getDateDefaultOperator(params),
      [params],
    );
    ...
    const changeOp = React.useCallback(
      (op: string) =>
        updateParams(computeOperatorParams(op, params, "minDate", "maxDate")),
      [updateParams, params],
    );
      ...
      (changed: Record<string, any>) => {
        if ("minDate" in changed || "maxDate" in changed) {
          if (operator === "eq" && "minDate" in changed) {
            updateParams({
              minDate: changed.minDate,
              maxDate: changed.minDate,
            });
          } else if (operator === "eq" && "maxDate" in changed) {
            updateParams({
              minDate: changed.maxDate,
              maxDate: changed.maxDate,
            });
          } else {
