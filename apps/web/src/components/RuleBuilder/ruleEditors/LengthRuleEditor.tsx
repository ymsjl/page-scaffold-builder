...
const LengthRuleEditor: React.FC<Props> = React.memo(
  ({ params, updateParams }) => {
    const lengthOp = React.useMemo(
      () => getLengthDefaultOperator(params),
      [params],
    );
    ...
    const handleOperatorChange = useCallback(
      (op: string) => {
        if (op === "eq") {
          const nextLen =
            params?.len ??
            (params?.min === params?.max ? params?.min : undefined);
          updateParams({
            len: nextLen,
            min: undefined,
            max: undefined,
