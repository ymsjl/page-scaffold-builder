import { type RuleNode, RuleNodeType, RuleParamsOfDecimalSchema } from '../RuleParamsDateSchema';
...
export class DecimalStrategy extends BaseStrategy {
  ...
  toRule(node: RuleNode, message: string): AntdRule {
    ...
    const { precision, decimals, scale } = RuleParamsOfDecimalSchema.parse(node.params);
    ...
    const validator = async (_rule: any, value: any) => {
      ...
      if (!text || Number.isNaN(Number(text))) return Promise.reject(new Error(message));
  ...
  applyFieldProps(node: RuleNode, fieldProps: Record<string, any>): void {
    ...
    if (typeof prec === 'number' && prec >= 0) {
      nextProps.precision = prec;
      nextProps.step = Number((1 / 10 ** prec).toFixed(prec));
    }
