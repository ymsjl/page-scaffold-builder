import { type RuleNode, RuleNodeType, DateRangeSpanParamsSchema } from '../RuleParamsDateSchema';
...
export class DateRangeSpanStrategy extends BaseStrategy {
  ...
  buildDefaultMessage(node: Pick<RuleNode, 'type' | 'params'>): string {
    if (!this.type) return '日期跨度不符合要求';
    const { success, data, error } = DateRangeSpanParamsSchema.safeParse(node.params);
  ...
  toRule(node: RuleNode, message: string): AntdRule {
    ...
    const { minSpan, maxSpan, operator } = DateRangeSpanParamsSchema.parse(node.params);
    const validator = async (_rule: any, value: any) => {
      ...
      if (operator === 'eq') {
        ...
        return span === target ? Promise.resolve() : Promise.reject(new Error(message));
      }
      if (operator === 'gte') {
        if (minSpan !== undefined && span < minSpan) return Promise.reject(new Error(message));
        return Promise.resolve();
      }
      if (operator === 'lte') {
        if (maxSpan !== undefined && span > maxSpan) return Promise.reject(new Error(message));
        return Promise.resolve();
      }
