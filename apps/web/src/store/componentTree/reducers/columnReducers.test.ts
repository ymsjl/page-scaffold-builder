...
const updateTableProps = (state: any, tableId: string, updates: Record<string, any>) => {
...
const addEntityModel = (state: any, entityModelId: string, fields: SchemaField[]) => {
...
const makeColumn = (key: string, title: string, dataIndex: string): ProCommonColumn => ({
  key,
  title,
  dataIndex,
});
...
describe('columnReducers', () => {
  it('adds columns from entity model and avoids duplicates', () => {
    ...
    const { columns } = nextState.normalizedTree.entities.nodes[tableId].props;
  ...
  it('applies editing column changes and inserts when missing', () => {
    ...
    expect(firstColumn.title).toBe('Age');
    expect(firstColumn.dataIndex).toBe('age');
    expect(firstColumn.valueType).toBe('text');
    expect(typeof firstColumn.key).toBe('string');
  });
  ...
  it('updates existing column when editing column matches', () => {
    ...
    expect(columns.length).toBe(1);
    const [firstColumn] = columns;
    expect(firstColumn.hideInTable).toBe(true);
  });
  ...
  it('upserts columns for selected node', () => {
    ...
    let nextState = componentTreeReducer(
      state,
      componentTreeActions.upsertColumnOfSelectedNode(makeColumn('col_1', 'Name', 'name')),
    );

    let columns;
    ({ columns } = nextState.normalizedTree.entities.nodes[tableId].props);
    expect(columns.length).toBe(1);
    const [firstColumn] = columns;
    expect(firstColumn.title).toBe('Name');
    ...
    expect(updatedColumn.title).toBe('Updated');
  });
  ...
  it('deletes and moves columns on selected node', () => {
    ...
    let columns;
    ({ columns } = nextState.normalizedTree.entities.nodes[tableId].props);
    expect(columns.map((col: ProCommonColumn) => col.key)).toEqual(['col_2', 'col_3', 'col_1']);
    ...
    ({ columns } = nextState.normalizedTree.entities.nodes[tableId].props);
    expect(columns.map((col: ProCommonColumn) => col.key)).toEqual(['col_3', 'col_1']);
  });
});
